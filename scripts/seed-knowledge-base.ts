 ...
 * 4개 패치를 8개 청크로 나눠 knowledge 테이블(듀얼 임베딩)에 삽입.
 *
...
업종코드 추가 권장: 소프트웨어 개발업(722) 추가하면 기술평가 점수 유리`,
  ...
  // ─── 패치4: 특허 ─────────────────────────────────────────────────────────────
  {
    ...
    category: "특허",
    content: `출원번호 10-2026-0082817, 심사청구 완료
...
async function main() {
  ...
  // 이 스크립트가 다루는 source 키만 초기화한다 — knowledge 테이블은 이제 PDF/wiki/web-learn 등
  // 다른 파이프라인 데이터도 함께 담고 있어서, 예전처럼 테이블 전체를 비우면 그 데이터까지
  // 전부 날아간다(통합 전 knowledge_base는 이 시드 데이터 전용이라 전체 삭제가 안전했음).
  for (const chunk of CHUNKS) {
    await deleteKnowledgeBySource(chunk.source);
  }
  console.log("기존 시드 데이터(source 매칭분만) 초기화 완료");
  ...
  for (let i = 0; i < CHUNKS.length; i++) {
    ...
    try {
      const result = await saveKnowledgeRows([
        { source: chunk.source, title: chunk.title, content: chunk.content, category: chunk.category },
      ]);
      if (result.saved === 0) throw new Error(result.openRouterError ?? result.voyageError ?? "알 수 없는 오류");
      console.log(` ✅`);
      success++;
    } catch (e) {

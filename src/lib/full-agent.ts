...
const TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "GitHub에 코드 변경 이슈를 생성한다. auto_implement 값에 따라 자동구현 파이프라인이 즉시 시작될 수도, 사람 검토를 거칠 수도 있다 — 아래 auto_implement 설명을 반드시 읽고 신중히 판단하라. 실행 전 CTO(기술 마케터) 검증을 거치며, 지시가 이상해 보이면 이슈를 만들지 않고 우려사항만 반환한다 — 그 경우 대장에게 반드시 재확인하라.",
    input_schema: {
      type: "object",
      properties: {
        title: { type: "string" },
  ...
  },
  {
    ...
    description:
      "점검보고서/견적서/완료확인서/안전안내문/계약서/제안서를 실제 업무용 문서로 작성해 PDF/Word 다운로드 링크를 만든다. 대장이 필요한 정보(고객명, 작업 내용, 금액 등)를 충분히 말하기 전까지는 먼저 되물어 정보를 모으고, 정보가 모이면 이 도구를 호출하라. 견적서/계약서/완료확인서/제안서는 CLO+CFO 검증을 통과해야 생성되며, 반려되면 사유가 포함된 오류가 반환된다 — 그 경우 대장에게 반려 사유를 전달하고 재확인하라.",
    input_schema: {
      type: "object",
      properties: {
        doc_type: {
...
## 지시 검증 게이트 — 자문단이 이의를 제기하면 반드시 멈춰라
create_content_draft/github_create_issue는 CMO(마케터)/CTO(기술 마케터) 관점의 검증을, generate_document는
견적서·계약서류에 한해 CLO+CFO 관점의 검증을 실행 직전에 거친다. 도구 결과에 "⚠️ ... 검토 결과
등록/이슈를 생성하지 않았습니다" 또는 "자문단 검토 반려(...)" 같은 문구가 포함돼 있으면, 그건 네가
대장의 요청을 잘못 이해했을 가능성이 있다는 신호다. 이 경우:
- 절대 같은 내용으로 도구를 다시 호출하거나 우회하지 마라.
- 검증에서 나온 우려 내용을 대장에게 그대로 전달하고, "이렇게 이해했는데 맞습니까?"라고
  명확히 재확인 질문을 하라.
- 대장이 확인·수정해주면 그때 다시 시도하라.
- create_video_job/create_blog_job의 "⚠️ COO 운영 검토" 메모는 하드 블록이 아니다 — 작업은
  이미 등록됐으니 그 내용을 참고로만 대장에게 전달하라(재확인 질문 불필요).

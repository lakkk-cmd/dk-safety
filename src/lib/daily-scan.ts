...
{
  "summary": "오늘 한 줄 요약",
  "anomalies": [{"title": "짧은 제목", "description": "구체적 설명과 근거"}],
  "opportunities": [{"title": "짧은 제목", "description": "구체적 설명과 근거"}]
}
\`\`\`
둘 다 없으면 각각 빈 배열로, summary에 "오늘 특이사항 없음"이라고 적는다. 각 배열은 최대 3개까지만.`;

export async function runDailyBusinessScan(): Promise<DailyBusinessScan> {
...
위 데이터를 분석해 오늘 아침 스캔 리포트를 작성하라.`;

  const raw = await callClaudeCustom(SYSTEM_PROMPT, prompt, 4000, 60_000);
  const jsonText = extractJsonBlock(raw);
  const parsed = jsonText
    ? (JSON.parse(jsonText) as { summary?: string; anomalies?: ScanItem[]; opportunities?: ScanItem[] })
    : { summary: raw.trim().slice(0, 300), anomalies: [], opportunities: [] };
  ...
  return data as DailyBusinessScan;
}

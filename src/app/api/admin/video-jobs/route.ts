...
import { approveVideoJob, listVideoJobs, rejectVideoJob } from "@/lib/video-jobs";
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (body.action === "approve") {
      const item = await approveVideoJob(body.id);
      return NextResponse.json({ item });
    }
    if (body.action === "reject") {
      const item = await rejectVideoJob(body.id, String(body.note ?? ""));
      return NextResponse.json({ item });
    }
    return NextResponse.json({ message: "action은 approve 또는 reject여야 합니다." }, { status: 400 });
  } catch (error) {
    return NextResponse.json(

...
export async function POST(request: Request) {
  ...
  let workDate: string, workTime: string, detail: string, totalAmount: number, workerId: string;
  ...
  if (!name || !phone || !address || !serviceType || !workDate || !workerId) {
    return NextResponse.json({ error: "필수 항목 누락 (고객명, 연락처, 주소, 작업종류, 작업일, 담당 기사)" }, { status: 400 });
  }

  try {
    const reservation = await pgCreateWalkInReservation({
      name, phone, address, serviceType, workDate, workTime: workTime || "00:00", detail, imageUrls, totalAmount, workerId
    });
    return NextResponse.json({ reservation }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "현장 접수 등록에 실패했습니다.";

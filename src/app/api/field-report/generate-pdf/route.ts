...
import { generateFieldReportPdfs } from "@/lib/field-report-pdf";
import { pgGetFieldReportForWorker, pgSaveFieldReportPdf } from "@/lib/field-reports";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!report.opinionLandlord || !report.opinionResident) {
      return NextResponse.json({ message: "소견 먼저 생성하세요." }, { status: 400 });
    }

    const { landlordUrl, residentUrl } = await generateFieldReportPdfs(report);
    const updated = await pgSaveFieldReportPdf(fieldReportId, { landlordUrl, residentUrl });

    return NextResponse.json({
      pdfLandlordUrl: updated.pdfLandlordUrl,
      pdfResidentUrl: updated.pdfResidentUrl,

  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const files = formData
        ...
        .slice(0, 5);
      if (files.length < 1) {
        return NextResponse.json({ message: "긴급출동 요청 시 현장 사진을 최소 1장 첨부해주세요." }, { status: 400 });
      }
      imageUrls = await saveImageFiles(files, "emergency");
    } else {
      return NextResponse.json({ message: "긴급출동 요청은 현장 사진 1장 이상 첨부가 필요합니다." }, { status: 400 });
    }

    const now = new Date();
    const today = now.toISOString().slice(0, 10);
    const immediateTime = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
    const scoreText =
    ...
    const message = error instanceof Error ? error.message : "긴급출동 요청 처리 중 오류가 발생했습니다.";

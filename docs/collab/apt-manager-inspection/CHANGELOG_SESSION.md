...
- CEO가 새 FAIL 이미지(`CEO_fail_confirm_unit_number_wrong_cell.png`)로 재지적: "501호"가
  "인" 칸에 찍혀 있다는 것.
- 원본을 8배율로 재확대해 직접 육안 확인 → 7회차의 자동 스캔(다크픽셀 비율) 기반 격자
  판단이 틀렸음을 발견. x=412.9는 실제 격자선이 아니라 "호" 글자 획 자체가 다수 y에서
  우연히 일관되게 어두워서 생긴 오탐이었다 — 실제 확인란은 세로선 3개(확인\|넓은 한 칸)
  뿐인 2칸 구조.
- 수정: 호수는 숫자만(호 재기입 금지), "호" 라벨 **뒤**가 아니라 **바로 앞**의 좁은
  여백(368.4~401.5pt)에 우측정렬로 스탬프하도록 반전. 서명은 "인" 바로 앞으로 이동.
  담당자(점검자명) 칸은 원래도 맞았으므로 변경 없음.
- `CONFIRM_TABLE` 상수 주석에 자동스캔 오탐 사실과 교훈(격자선 확정은 고배율 육안
  교차검증 필수)을 명시.
- `crop-confirm-cells.png`(+unvisited) 재생성 — "203 호"/"501 호"로 숫자·라벨 겹침 없음,
  "호" 뒤 공간엔 서명(방문)/공란(미방문)만 있음을 확인.
- `npm run build` 재통과, 방문·미방문 샘플 + 비교 PNG 전부 재생성.
- `PATH_A_COORDINATES.md`에 오탐 경위와 교훈 명시하며 갱신.
- **여기서 다시 중단** — CEO/총괄 최종 확정 승인 대기.

## 2026-09-22 (Claude Code, 로컬) — 7회차: CRITICAL FAIL 2건 근본 수정 (격자선 픽셀 실측)

- CEO가 FAIL 근거 이미지 2장(`CEO_fail_remarks_overflow.png`, `CEO_fail_confirm_cells.png`)을
  첨부해 재지적 — 열람 확인함.
- 근본 원인 진단: 지금까지의 모든 좌표(비고 열, 확인란)는 텍스트 라벨 위치로 어림짐작한
  값이었고, 실제 **칸 경계(격자선) 자체를 측정한 적이 없었다**.
- `@napi-rs/canvas`+`pdfjs-dist`로 원본을 4배율 라스터 → 특정 구간에서 항상 어두운 x/y
  좌표(다크비율 0.85~1.00)를 찾는 방식으로 격자선을 직접 실측:
  - 비고 열: 좌 506.8pt / 우 544.5pt (실사용폭 37.7pt) — 기존 텍스트 시작점(505)이
    좌측 테두리보다 1.8pt 왼쪽이었음이 드러남
  - 확인란 미니테이블: 세로선 342.9/368.4/**412.9**/508.4, 가로선 146.8/128.3/112.3 —
    기존 담당자 칸(x=408)이 실제 셀 경계(412.9)보다 왼쪽이라 라벨 칸을 침범 중이었음
- 좌표 수정: `REMARK_COLUMN_X` 505→509.5, `INSPECTOR_NAME_BLANK` x 408→417·endX 495→492,
  `RESIDENT_CONFIRM_UNIT_LABEL` x 413→417, `RESIDENT_SIGNATURE_BOX` 재조정.
- 객관적 검증 스크립트 작성: 렌더링한 결과를 다시 4배율 라스터해 각 경계 바깥 픽셀 수를
  직접 세어 넘침 0을 확인(육안 아님) — 라벨 텍스트 오탐 구간은 원본 템플릿과 픽셀수
  대조로 걸러냄(129/94개, 완전 동일 = 원본 라벨일 뿐 침범 아님 확인).
  crop-remarks-inside-cell.png / crop-confirm-cells.png(+unvisited) 확대 크롭으로 육안
  증빙도 추가.
- 방문·미방문 샘플 + 원본 대비 PNG 재생성, `npm run build` 재통과.
- `PATH_A_COORDINATES.md`를 이번 실측값 기준으로 전면 갱신(격자선 좌표 + 검증 방법론 명문화).
- **여기서 다시 중단** — CEO/총괄 최종 확정 승인 대기.

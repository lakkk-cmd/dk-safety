...
## 접지저항 실측 입력 + 자동판정 신설 (2026-09-23, PR #53, 완전 검증 완료)

### 요청
부하전류/누설전류/절연저항처럼 접지저항도 실측 입력받아 PDF 기타사항에 넣고, 점검앱에서
입력되게 하고, AI진단·점검표 결과/비고란까지 전부 자동 연동해 달라는 요청. 기준값도 찾아서
자동판정에 반영.

### 기준값 결정
조사 결과 두 후보(구 전기설비기술기준 제3종접지공사 100Ω 이하 vs 신KEC 감전보호용
등전위본딩 공식)가 있어 대표님께 직접 확인 → **신KEC 공식**으로 확정: 접지저항 × 누전차단기
정격감도전류(30mA 고감도형) ≤ 허용접촉전압(50V) → 접지저항 ≤ 1,666.7Ω. 절연저항/누설전류와
달리 회로수와 무관한 전국 공통 고정기준(접지극 단일 물리량이라 병렬합성/합산 개념이 적용
안 됨).

### 구현 범위
마이그레이션 129(grounding_resistance 컬럼, 프로덕션 적용 완료) + 규칙엔진(자동판정 함수,
체크리스트 항목을 수동확인→자동판정 전환) + worker/apt-manager 점검등록 API + 관리자
pdf/reissue-pdf API + PDF 1페이지(기타사항+점검결과·비고) + PDF 2페이지(기존 하드코딩
placeholder였던 "접지저항: 미측정" 행을 실제 계산값으로 교체) + AI 안전진단(실측값 해설+
시스템 프롬프트) + 점검앱 입력폼(정상모드+세대미방문 연속입력모드 둘 다, 워커·전기과장 공용
컴포넌트라 한 곳 수정으로 양쪽 다 적용) + 관리자 상세보기 화면까지 전부 반영.

### 배포 중 발견·수정한 CI 실패
1차 배포 시도에서 `document-pdf.tsx`의 `UnitInspectionPdfData` 타입(모든 라우트가 공유하는
공개 타입)에 새 필드 추가를 빠뜨려 CI 빌드가 타입에러로 실패 — 로컬 빌드는 통과했다고
판단했으나 재확인 결과 실제로는 다른 파일(레거시 스크립트)에서도 같은 타입 에러가 있었고,
빌드 로그를 파일로 저장해 직접 grep으로 확인하는 절차로 바꿔 두 에러 모두 잡아 수정 후
재배포 성공.

### 검증 (실제 프로덕션, 시뮬레이션 아님)
- 마이그레이션 129 프로덕션 적용 확인(information_schema).
- 시연전용아파트 데모 계정(실고객 아님. 로그인 값이 기존 기록과 달라져 있어 DB에서
  프로덕션 해시 함수로 맞춘 뒤 로그인)으로 **실제 프로덕션 API에 접지저항 2100Ω(기준 초과,
  부적합 유도) 점검을 실제 제출**. 계정 아이디·비밀번호 원문은 이 문서에 적지 않는다.
- 응답 JSON에서 확인: `grounding_panel_resistance` 항목이 `"result":"X"`,
  `"note":"실측 2100Ω (기준 1666.7Ω 초과 시 부적합)"`로 정확히 자동판정됨,
  `autoDiagnosis` 배열에도 정확히 반영됨.
- 발급된 PDF를 실제 다운로드해 확인: 1페이지 기타사항 "접지 2100Ω" 포함, 점검결과 ×
  표기 정상, 2페이지 "실측과 기준 비교" 표에 "접지저항 실측 2100Ω 기준 1666.7Ω초과
  기준초과" 행이 (기존 하드코딩 placeholder 대신) 실제 계산값으로 정상 표시.
- AI 안전진단 백그라운드 완성 후 정정본을 재다운로드해 확인: 우선순위·종합총평·권고사항
  전부 접지저항 2100Ω 초과를 정확히 서술("분전반 접지저항이 기준을 초과하였으니 전문
  업체 상담을 권합니다" 등) — 이전 세션에서 고친 "우선순위 소수점 절단 버그"도 여기서
  재확인(전체 문장이 안 잘리고 온전히 나옴).
- 테스트로 만든 점검 기록·AI진단·정정본 포인터·CRM 상담로그는 검증 후 전부 SQL로 삭제
  (Storage 파일 자체는 정리 안 함, 부수적).
- `npm run build`/`npm run lint` 통과(1차 실패 원인 수정 후 재확인).

### 배포 결과
- PR: https://github.com/lakkk-cmd/dk-safety/pull/53 (MERGED)
- 커밋(main): `10acbeea20e722a4fe3198f096840ae7904e101d` (short `10acbee`)
- Vercel production 배포: Ready 확인, `https://dkansim.com/` 200 확인

결제·불가역 삭제 없음. 데모 계정 비밀번호는 검증용으로 재설정했으나 원문은 기록하지 않음
(실고객 계정 아님, 대표님 실사용에 영향 없음). 이 문장 이전 커밋에 비밀번호가 평문으로
남아 있으면 그 커밋은 공개 저장소 이력에 있으므로, 데모 계정 비밀번호는 다시 바꿔야 한다.

...
- 시연전용아파트 데모 계정(로그인 admin/admin, 비밀번호가 문서와 달라져 있어 DB에서
  프로덕션 해시 함수로 직접 재설정 후 로그인 — 데모 계정이라 안전하게 가능)으로 **실제
  프로덕션 API에 접지저항 2100Ω(기준 초과, 부적합 유도) 점검을 실제 제출**.
...
시크릿 노출·결제·불가역 삭제 없음. 데모 계정 비밀번호를 문서 기준값(admin)으로 재설정했음
(실고객 계정 아님, 대표님 실사용에 영향 없음).

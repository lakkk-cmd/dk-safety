...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='electrical_tips'
      AND policyname='electrical_tips_public_read'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='electrical_tips'
      AND policyname='electrical_tips_admin_all'
  ) THEN
...
DROP TRIGGER IF EXISTS trg_electrical_tips_updated_at ON public.electrical_tips;
...
-- 초기 샘플 데이터 3건 (테이블이 비어있을 때만 — 재실행 시 중복 삽입 방지)
...
) AS t(category, title, summary, content, service_type, is_published, display_order)
WHERE NOT EXISTS (SELECT 1 FROM public.electrical_tips);

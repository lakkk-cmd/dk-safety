-- KEC/전기안전관리법 규정 지식베이스 시드 (category='regulation')
--
-- 주의: knowledge_base.embedding은 vector(1536) 컬럼으로, OpenRouter
-- text-embedding-3-small 임베딩 값이 필요해 순수 SQL로는 채울 수 없다.
-- 이 파일은 실제로 삽입된 7개 청크의 내용을 문서/버전관리 목적으로 보존한다.
-- 실제 DB 반영(임베딩 생성 + INSERT)은 scripts/seed-regulation-knowledge.mjs로 수행한다:
--   node --env-file=.env.local scripts/seed-regulation-knowledge.mjs

-- 1. KEC 210 저압 옥내배선
insert into knowledge_base (source, title, content, category, is_external) values (
  'KEC 210',
  'KEC 210 저압 옥내배선 — 전선 굵기·허용전류·보호방식',
...
insert into knowledge_base (source, title, content, category, is_external) values (
  'KEC 232',
  'KEC 232 콘센트 시설 기준 — 접지형·방우형·설치 높이',
...
insert into knowledge_base (source, title, content, category, is_external) values (
  'KEC 234',
  'KEC 234 누전차단기 — 주거용 정격감도전류 30mA 이하 의무',
...
insert into knowledge_base (source, title, content, category, is_external) values (
  '공동주택 화재통계',
  '공동주택 화재 원인 TOP5 (아크/절연불량/과부하/접촉불량/누전) 및 계측 판단 기준값',
...
insert into knowledge_base (source, title, content, category, is_external) values (
  '제조사 권장기준',
  '차단기/콘센트 교체 권장 연한 (15~20년, 제조사 공통 기준)',

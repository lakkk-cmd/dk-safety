...
| `src/lib/document-pdf.tsx` | next/og(satori) JSX → PNG → pdf-lib A4-slice PDF rendering — `renderDocumentPdf()` for customer-facing docs (견적서 등, letterhead+sections), `renderAdminReportPdf()` (2026-08-17) for internal reports using the 2025 행정업무운영편람 간이기안문(별지 제2호서식) layout (생산등록번호/등록일/결재일/공개구분 + 기안·검토·협조·결재란 grid + title/summary + org label + body), `parseMarkdownSections()` shared by both |
...
- `/hq`, `/hq/login` — AI executive command center (cookie-protected, shares admin auth); served at `hq.dkansim.com` via host-based middleware rewrite; tabs(2026-09 재설계): 홈/AI 채팅/SWOT은 단독 탭, 나머지는 **운영**(예약·보고서)/**콘텐츠**(카카오·영상 제작·블로그 원고·인텔리전스) 드롭다운 그룹으로 재배치(`hq-shell.tsx`) — 직접 접속(호스트 재작성 없는 환경)에서도 링크가 깨지지 않도록 `src/lib/hq-links.ts`의 `hqBasePath()`가 `/hq` 접두사를 필요할 때만 붙인다. `/hq` 루트는 3단 위계 대시보드(오늘수익 → 이상신호[평소엔 조용, 문제 있을 때만 배너] → 확인·결정 필요[콘텐츠승인·대장피드백·개선요청·`대표님이 해야할 일`] → 배경상태[파이프라인·아침스캔·주간점검, 접힌 카드]), `/hq/chat`이 9-에이전트 채팅(`src/app/hq/chat/chat-client.tsx`) 전용 화면 — `hq-shell.tsx`가 `/chat` 경로에서만 풀높이(`h-dvh`) 레이아웃을 적용한다
...
- `/hq/swot` — SWOT 분석 + TOWS(SO/WO/ST/WT) 전략 매트릭스(2026-09-06 신설, `src/lib/swot-analysis.ts`) — 분기별 자동 재생성(`swot-analysis` 크론, 1/4/7/10월 1일) + 화면의 "지금 재생성" 버튼으로 수동 실행도 가능(`POST /api/admin/swot`). TOWS에서 뽑힌 대표님 액션아이템은 홈 화면의 "대표님이 해야할 일"과 **같은 테이블**(`report_action_items`)을 공유한다 — 이 테이블은 기존엔 `report_id`(agent_reports 필수 참조)만 있었는데, `swot_analysis_id` nullable 컬럼을 추가하고 둘 중 정확히 하나만 채워지도록 CHECK 제약을 걸었다(120_swot_analysis.sql). `ActionItemsCard` 컴포넌트가 출처와 무관하게 동일하게 렌더링한다.
- `/hq/kakao` (2026-09-06 신설, `src/components/hq/kakao-approval-panel.tsx`) — 카카오 채널 포스트 승인만 hq 안에서 가능하게 함(대표님이 `/hq`에서 카카오 승인 화면을 못 찾겠다고 한 게 계기, 그 전엔 contents.dkansim.com에만 있었음). **처음엔 유튜브/카카오/블로그를 전부 합친 화면(`/contents`의 `ContentApprovalPanel` 재사용)으로 만들었다가, "카카오만 승인 가능해야 한다"는 지시로 카카오 전용 컴포넌트+전용 API(`GET /api/admin/content/kakao`, `listKakaoQueueForApproval()` in content-pipeline.ts)로 다시 만들었다** — 유튜브·블로그는 이 화면에 절대 나타나지 않는다. `/hq/videos`·`/hq/blog-jobs`(job_id 기반 dk-video/blog-factory 파이프라인)와도 완전히 별개 시스템이니 혼동 주의.
...
- `/contents` — content marketing command center: YouTube/Kakao/blog approval queues, Naver trend keywords, YouTube OAuth connection status (cookie-protected, shares admin auth); served at `contents.dkansim.com` via host-based middleware rewrite; 카카오만 별도로 `/hq/kakao`에도 있음(전용 컴포넌트, 유튜브·블로그는 없음)
...
- `119` — `report_action_items` table (경영진 보고서에서 뽑은 "대표님이 해야할 일" 체크박스+메모,
  홈 화면 통합 할일목록; 원래는 `report_id`로 `agent_reports`만 참조 가능했음)
- `120` — `swot_analyses` table(분기별 SWOT/TOWS 전략 분석) + `report_action_items.report_id`를
  nullable로 완화하고 `swot_analysis_id`를 추가, 둘 중 정확히 하나만 채워지도록 CHECK 제약
  (`report_action_items_source_check`) — SWOT 기원 액션아이템도 같은 체크리스트에 통합하기 위함
- `121` — `agent_memory_entries` table(조직 통합 기억) 신설 + `agent_memory`(주간회의 전용
  구조화 JSON)와 `agent_shared_memory`(채팅 전용 스트림)의 기존 데이터를 category 태그(`decision`/
  `open_question`/`kpi`/`theme`/`meeting_summary`/`note`)를 붙여 전부 백필. 두 테이블을 대체함

### AI Command Center

`hq.dkansim.com` (internally `/hq`) runs a virtual 6-executive meeting (CTO, CSO, CMO, COO, CFO, CLO) powered by `ANTHROPIC_API_KEY`. Agent system prompts are in `src/lib/agents.ts`. Meeting schedule and topics persist in Supabase (`agent_memories` table). The model used is controlled by `ANTHROPIC_MODEL` env var (defaults to `claude-sonnet-4-6`). Reports approved for content use appear in the `report.dkansim.com` (`/report`) archive. Every weekly meeting report (`/api/cron/morning-report`) is also rendered as a PDF in the 행정업무운영편람 간이기안문 format (2026-08-17, `renderAdminReportPdf` in `document-pdf.tsx`) and attached to the email + linked from `agent_reports.pdf_url`; the same format is available on demand in chat via `generate_document(doc_type: "admin_report")`.

**조직 통합 기억** (`src/lib/org-memory.ts`, 2026-09-06 통합) — 주간회의(decision/open_question/kpi/theme/meeting_summary)와 9-에이전트 채팅(note, `buildBusinessSnapshot()`을 통해 daily-scan·SWOT분석·총괄디렉터 전부가 참조)이 `agent_memory_entries` 하나를 공유한다. 통합 전엔 `agent_memory`(주간회의 전용)와 `agent_shared_memory`(채팅 전용)가 완전히 분리돼 있어서, 대표님이 한쪽 대화(예: 총괄디렉터 채팅)에서 확정한 결정을 다른 쪽(주간회의·daily-scan)이 전혀 모르는 문제가 실제로 반복됐다(발견 계기: "겸업조항 확인 완료"를 채팅에는 기록했는데 daily-scan은 몰랐던 사례). 총괄디렉터는 `remember_decision` 도구로 확정된 결정·정책을 직접 영구 기록할 수 있다 — "기억했습니다"라고 답만 하고 도구를 안 부르면 이 대화 기록에만 남고 다른 시스템에는 전달되지 않는다(시스템 프롬프트 규칙 8번).
...
`/hq/swot` (SWOT/TOWS 분기 전략분석, `src/lib/swot-analysis.ts`, 2026-09-06 신설) — `buildBusinessSnapshot()` + 조직 통합 기억(`org-memory.ts`) + 최근 90일 학습신호(`loadRecentSignalsBrief(90)`)를 근거로 Claude가 SWOT(강점/약점=내부, 기회/위협=외부) + TOWS(SO/WO/ST/WT) 전략을 JSON으로 생성, `swot_analyses`에 저장하고 TOWS에서 뽑힌 대표님 액션아이템은 `report_action_items`(`swot_analysis_id`)에 함께 저장한다. 분기별 크론(`/api/cron/swot-analysis`, 1/4/7/10월 1일) + 화면의 수동 재생성 버튼(`POST /api/admin/swot`) 둘 다로 실행 가능.

**2026-09-06 발견·수정된 경영진 회의 파이프라인 버그 2건** (SWOT 신설과 같은 세션에서 발견): (1) `runFullMeeting`의 chief 호출 `maxTokens`가 1200이라 공문서형 보고서 본문을 쓰다 뒤쪽 조직기억 JSON 블록(`strategicThemes`/`decisions`/`openQuestions`/`kpis`)에 도달하기 전에 매번 잘려, 13주 내내 이 네 필드가 전부 빈 채로 방치됐었음(6000/90초로 상향해 해결) — `recentMeetings`(회의 요약 원문, 최근 5건만)만으로 다음 회의에 맥락이 전달되던 상태였다. (2) `daily-business-scan`이 자기 자신의 실행중 `pipeline_logs` 'started' 행을 좀비로 오인해 최소 16일 연속 거짓 이상신호를 보고 — 크론에서 그 행을 조사 완료 후에 기록하도록 순서를 바꿔 해결(`src/app/api/cron/daily-business-scan/route.ts`). 이 두 오진 및 "`agent_reports.approved`는 콘텐츠 노출용일 뿐 학습과 무관"이라는 사실은 `daily-scan.ts`/`swot-analysis.ts` 시스템 프롬프트에 "다시 꺼내지 말 것"으로 명시돼 있음 — 과거 회의요약/스캔이력에 잔재가 남아있어 향후 파이프라인이 다시 잘못 인용할 수 있기 때문.

...
- `/hq`, `/hq/login` — AI executive command center (cookie-protected, shares admin auth); served at `hq.dkansim.com` via host-based middleware rewrite; tabs(2026-09 재설계): 홈/AI 채팅/SWOT은 단독 탭, 나머지는 **운영**(예약·보고서)/**콘텐츠**(영상·블로그·인텔리전스) 드롭다운 그룹으로 재배치(`hq-shell.tsx`) — 직접 접속(호스트 재작성 없는 환경)에서도 링크가 깨지지 않도록 `src/lib/hq-links.ts`의 `hqBasePath()`가 `/hq` 접두사를 필요할 때만 붙인다. `/hq` 루트는 3단 위계 대시보드(오늘수익 → 이상신호[평소엔 조용, 문제 있을 때만 배너] → 확인·결정 필요[콘텐츠승인·대장피드백·개선요청·`대표님이 해야할 일`] → 배경상태[파이프라인·아침스캔·주간점검, 접힌 카드]), `/hq/chat`이 9-에이전트 채팅(`src/app/hq/chat/chat-client.tsx`) 전용 화면 — `hq-shell.tsx`가 `/chat` 경로에서만 풀높이(`h-dvh`) 레이아웃을 적용한다
...
`/hq/swot` (SWOT/TOWS 분기 전략분석, `src/lib/swot-analysis.ts`, 2026-09-06 신설) — `buildBusinessSnapshot()` + 누적 조직 기억(`agent_memory`) + 최근 90일 학습신호(`loadRecentSignalsBrief(90)`)를 근거로 Claude가 SWOT(강점/약점=내부, 기회/위협=외부) + TOWS(SO/WO/ST/WT) 전략을 JSON으로 생성, `swot_analyses`에 저장하고 TOWS에서 뽑힌 대표님 액션아이템은 `report_action_items`(`swot_analysis_id`)에 함께 저장한다. 분기별 크론(`/api/cron/swot-analysis`, 1/4/7/10월 1일) + 화면의 수동 재생성 버튼(`POST /api/admin/swot`) 둘 다로 실행 가능.
